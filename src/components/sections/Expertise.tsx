...
// SVG icons kept inline — matched by index to dict.expertise.areas
const areaIcons = [
  (
...
const areaColors = ['var(--teal)', 'var(--gold)', 'var(--teal-dark)', 'var(--gold-light)'];
    ...
    <section id="expertise" className="py-20 bg-white">
      <div className="max-w-7xl mx-auto px-6">
        ...
        <AnimateOnScroll className="text-center mb-16">
          <p className="text-[var(--teal)] font-semibold mb-2">{expertise.sectionLabel}</p>
        ...
        <div className="grid md:grid-cols-2 gap-8">
          {(expertise.areas as Array<{ title: string; description: string }>).map((area, index) => {
            const color = areaColors[index] ?? 'var(--teal)';
            return (
              <AnimateOnScroll
                key={index}
                delay={index * 0.12}
        ...
        <AnimateOnScroll delay={0.2} direction="up">
          ...
          >
            <h3 className="text-2xl md:text-3xl font-bold mb-4">{expertise.missionTitle}</h3>
            <p
              className="text-lg md:text-xl max-w-4xl mx-auto leading-relaxed opacity-95"
              dangerouslySetInnerHTML={{ __html: expertise.missionBody }}
            />
            <p className="mt-6 text-[var(--gold-light)] font-semibold text-lg">
              {expertise.missionTagline}
            </p>
          </motion.div>
        </AnimateOnScroll>
      </div>
